...
export const getTopStoresBySales = async (region, industry, limit) => {
  try {
    const response = await apiClient.get("/topStoresBySales", {
      params: {
        region,
        industry,
        limit,
...
export const getIndustryAvgSalesInfo = async (region) => {
  try {
    const response = await apiClient.get("/industryAvgSalesInfo", {
      params: {
        region,
      },
    });
...
export const getLowestFranchiseFeeStores = async (region, industry, limit) => {
  try {
    const response = await apiClient.get("/lowestFranchiseFeeStores", {
      params: {
        region,
        industry,
        limit,
...
export const getIndustryNewOpeningsCount = async () => {
  try {
    const response = await apiClient.get("/industryNewOpeningsCount", {
      params: {},
    });
    return response.data; // 서버로부터의 응답 데이터 반환
  } catch (error) {
